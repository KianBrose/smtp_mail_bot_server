...
def sanitized_log(func: Callable[..., None], msg: AnyStr, *args, **kwargs) -> None:
    ...
    """
    sanitized_args = [sanitize(a) if isinstance(a, bytes) else a for a in args]
...
class SMTP(asyncio.StreamReaderProtocol):
    ...
    def __init__(
        ...
        *,
        ...
        if tls_context:
            if tls_context.verify_mode not in {
                ssl.CERT_NONE,
                ssl.CERT_OPTIONAL,
            }:  # noqa: DUO122
                log.warning(
                    "tls_context.verify_mode not in {CERT_NONE, "
    ...
    def connection_made(self, transport: asyncio.transports.BaseTransport) -> None:
        ...
        if self.transport is not None and seen_starttls:
            ...
            if hook is None:
                ...
                self._tls_handshake_okay = hook(self, self.session, self.envelope)
        else:
    ...
    async def push(self, status: AnyStr):
        if isinstance(status, str):
            response = bytes(status, "utf-8" if self.enable_SMTPUTF8 else "ascii")
        else:
            response = status
    ...
    async def _handle_client(self):
        ...
        if self._enforce_call_limit:
            call_limit = collections.defaultdict(
                lambda x=self._call_limit_default: x, self._call_limit_base
            )
        else:
    ...
    async def smtp_EHLO(self, hostname: str):
        ...
        if hasattr(self, "ehlo_hook"):
            warn(
                "Use handler.handle_EHLO() instead of .ehlo_hook()", DeprecationWarning
            )
    ...
    async def smtp_STARTTLS(self, arg: str):
        ...
        self._tls_protocol = sslproto.SSLProtocol(
            self.loop, self, self.tls_context, waiter, server_side=True
        )
    ...
    async def smtp_RSET(self, arg: str):
        ...
        if hasattr(self, "rset_hook"):
            warn(
                "Use handler.handle_RSET() instead of .rset_hook()", DeprecationWarning
            )
